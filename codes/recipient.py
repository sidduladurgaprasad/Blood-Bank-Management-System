...
class recipient:
    def __init__(self,root):
        ...
        bg_img.place(x=0,y=0,width=1580,height=800)
        
        #label placed on background
        tlabel=Label(bg_img,text="RECIPIENT DETAILS",font=("times new roman",40,"bold"),bg="GREEN",fg="white")
        tlabel.place(x=0,y=0,width=1580,height=80)

        main_frame=Frame(bg_img,bd=2)
        main_frame.place(x=80,y=140,width=1050,height=580)

        #left label frame
        lframe=LabelFrame(main_frame,bd=2,relief=RIDGE,text="RECIPIENT DETAILS",font=50)
        lframe.place(x=5,y=10,width=485,height=560)
        img=Image.open(r"D:\PROJECTS\images\rec1.jpg")
        ...
        img2=Image.open(r"D:\PROJECTS\images\details2.jpg")
    ...
    def add_data(self):
        if (self.var_n.get()=="" or self.var_a.get()=="" or self.var_id.get()=="" or self.var_bg.get()=="Select Blood Group" or self.var_p.get()=="" or self.var_q.get()==""):
            ...
            try:
                ...
                mycursor=con.cursor()
                mycursor.execute("insert into recipient values(%s,%s,%s,%s,%s,%s,'pending')",(
                                                                                    self.var_n.get(),
                                                                                    self.var_a.get(),
                                                                                    self.var_id.get(),
                                                                                    self.var_bg.get(),
                ...
                self.fetch_data()
                con.close()
                messagebox,messagebox.showinfo("SUCCESS","Recipient Details has been added Successfully",parent=self.root)                                                                                                                       
    ...
    def update_data(self):
        if (self.var_n.get()=="" or self.var_id.get()=="" or self.var_a.get()=="" or self.var_bg.get()=="Select Blood Group" or self.var_p.get()=="" or self.var_q.get()==""):
            ...
            try:
                Update=messagebox.askyesno("update","Do you want to update this recipient details",parent=self.root)
                if Update>0:
                    con=mysql.connector.connect(host="localhost",username="root",passwd="Sarthan@123",database="blood_bank")
                    mycursor=con.cursor()
                    mycursor.execute("update recipient set recipientname=%s,age=%s,bloodgroup=%s,phoneno=%s,quantity=%s,status='pending' where recipientid=%s",(
                                                                                                                                         self.var_n.get(),
                                                                                                                                         self.var_a.get(),
                                                                                                                                         self.var_bg.get(),
                                                                                                                                         self.var_p.get(),
                    ...
                    if not Update:
                        return
                messagebox.showinfo("SUCCESS","Recipient details updated successfully",parent=self.root)
                con.commit()
                self.fetch_data()
                con.close()
            except Exception as es:
    ...
    def delete_data(self):
        if self.var_id.get()=="":
            messagebox.showerror("ERROR","RecipientID must be requried",parent=self.root)
        else:
            try:
                de=messagebox.askyesno("DELETE PAGE","Do you want to delete this recipient details",parent=self.root)
                if de>0:
                    con=mysql.connector.connect(host="localhost",username="root",passwd="Sarthan@123",database="blood_bank")
                    mycursor=con.cursor()
                    sql="delete from recipient where recipientid=%s"
                    val=(self.var_id.get(),)
                ...
                self.fetch_data()
                con.close()
                messagebox.showinfo("DELETE","Successfully deleted recipient details",parent=self.root)
            except Exception as es:
                messagebox.showerror("ERROR",f"Due To:{str(es)}",parent=self.root)
